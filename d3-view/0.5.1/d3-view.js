// d3-view Version 0.5.1. Copyright 2017 quantmind.com.
...
function defaultDebug(msg) {
    providers.logger.info(prefix$2 + ' ' + msg);
}
...
function reactive(model, key, value) {
    ...
    function property() {
        ...
        if (isObject$1(value) && isFunction(value.get)) {
            lazy = value;
...
function addTarget(modelEvents, model$$1, attr) {
    ...
    //
    if (isFunction(value) || arguments.length === 2) target.events.add('');else if (value instanceof model) {
        target.events.add('');
...
function createComponent(name, o, prototype, coreDirectives) {
    ...
    function Component(options) {
        ...
        this.model = objectAssign$1({}, isFunction(model) ? model() : model, pop(options, 'model'));
        this.init(options);
    }
...
function asView(vm, element, onMounted) {
    ...
    var p = d3Selection.select(element).view(vm).mount(null, onMounted);
...
var prefix$4 = '[d3-form]';

var warn$1 = function (msg) {
    providers$1.logger.warn(prefix$4 + ' ' + msg);
};

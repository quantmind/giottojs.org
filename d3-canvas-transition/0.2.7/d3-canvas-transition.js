...
var canvasListener = function (event) {
...
function mousemoveEvent(context, nodes, event) {
    ...
    if (actives) {
        for (i = 0; i < actives.length; ++i) {
            node = actives[i];
            if (active && node !== active && node.parentNode === active.parentNode) trigger(node, event, 'mouseleave');else if (nodes.indexOf(node) > -1) newActives.push(node);else trigger(node, event, 'mouseleave');
        }
    }
    ...
    for (i = 0; i < nodes.length; ++i) {
        node = nodes[i];
        if (actives.indexOf(node) > -1) trigger(node, event, 'mouseover');else if (node === active || node.parentNode !== active.parentNode) {
            actives.push(node);

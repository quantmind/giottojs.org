// d3-view Version 0.7.3. Copyright 2017 quantmind.com.
...
function reactive(model, key, value) {
    ...
    function typeValue(newValue, oldValue) {
        ...
        if (d3Let.isArray(newValue)) return arrayValue(newValue, oldValue);else if (isModel(oldValue)) return modelValue(newValue, oldValue);else return isVanillaObject(newValue) ? model.$new(newValue) : newValue;
    }

    function arrayValue(newValue, oldValue) {
        if (d3Let.isArray(oldValue)) newValue.forEach(function (o, i) {

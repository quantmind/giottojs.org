// d3-view Version 1.1.0. Copyright 2017 quantmind.com.
...
var base$2 = {
    ...
    renderFromUrl: function renderFromUrl(url, context) {
        ...
        return this.fetchText(url).then(function (text) {
            cache[url] = text;
...
function reactive(model, key, value) {
    ...
    function update(newValue) {
        ...
        if (providers.logger.debug) providers.logger.debug('[d3-model] updating ' + modelName + '.' + key);
...
function propagate(model, name, data) {
    if (!model) return;
    if (d3Let.isFunction(model[name])) model[name](data);

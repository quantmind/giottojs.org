...
var line$1 = createChart('linechart', lineDrawing, {
    ...
    doDraw: function doDraw(frame) {
        ...
        line$$1 = d3_shape.line().x(xl).y(yl).curve(this.curve(model.curve));
        ...
        lines.enter().append('path').attr('class', 'line').attr('fill', 'none').attr('stroke', stroke).attr('stroke-width', model.lineWidth).merge(lines).transition().attr('stroke', stroke).attr('stroke-width', model.lineWidth).attr('d', line$$1);
...
var barChartPrototype = {
    ...
    stacked: function stacked(bars, data, groups) {
        ...
        var stackOrder = d3_shape.stackOrderDescending,
        ...
        data = d3_shape.stack().order(stackOrder).keys(groups)(data);

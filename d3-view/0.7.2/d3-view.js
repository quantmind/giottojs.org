// d3-view Version 0.7.2. Copyright 2017 quantmind.com.
...
function reactive(model, key, value) {
    ...
    function typeValue(newValue, oldValue) {
        ...
        if (d3Let.isArray(newValue)) return arrayValue(newValue, oldValue);else if (isModel(oldValue)) return modelValue(newValue, oldValue);else return isObject$1(newValue) ? model.$new(newValue) : newValue;
    }

    function arrayValue(newValue, oldValue) {
        if (isModel(oldValue)) oldValue.$off();else if (d3Let.isArray(oldValue)) newValue.forEach(function (o, i) {
...
function addTarget(modelEvents, model$$1, attr) {
    ...
    // value is another model, events at both target model level and value model level
            ...
            if (!model$$1.$events.has(attr)) model$$1.$set(attr, null);

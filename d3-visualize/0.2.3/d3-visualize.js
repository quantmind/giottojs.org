// d3-visualize Version 0.2.3. Copyright 2017 quantmind.com.
...
}(this, (function (exports,assign,d3Array,d3Random,d3Collection,d3Let,d3View,d3Dsv,d3Dispatch,crossfilter,d3TimeFormat,d3Selection,d3Transition,d3_scale,d3Format,d3Axis,d3Shape,d3Color,d3SvgLegend) { 'use strict';
...
var Svg = createPaper('svg', {
    initialise: function initialise(visual) {
        var svg = visual.paperElement.select('svg#' + visual.model.uid);
        if (!svg.size()) svg = visual.paperElement.append('svg').attr('id', visual.model.uid).classed(visual.visualType, true).style('position', 'absolute');
        return svg.node();
    }
});
...
var Div = createPaper('div', {
    initialise: function initialise(visual) {
        ...
        if (!div.size()) div = visual.paperElement.append('div').attr('id', visual.model.uid).classed(visual.visualType, true);
...
var bar = createChart('barchart', lineDrawing, {
    ...
    doDraw: function doDraw(frame) {
        ...
        //
        // Stacked bar chart
        if (stacked || !groups) barChart.stacked(bars, data, groups);else barChart.grouped(bars, data, groups);
...
var barChartPrototype = {
    ...
    stacked: function stacked(bars, data, groups) {
        // TODO: generalize this
        var stackOrder = d3Shape.stackOrderDescending,
            color$$1 = this.viz.getModel('color'),
            sx = this.sx,
            sy = this.sy,
            sz = this.sz,
            x = this.model.x,
        ...
        data = d3Shape.stack().order(stackOrder).keys(groups)(data);
    ...
    grouped: function grouped(bars, data, groups) {
...
var pyramid$1 = createChart('pyramidchart', proportional, {
    ...
    doDraw: function doDraw(frame) {
        var model = this.getModel(),
            ...
            color$$1 = this.getModel('color'),
            box = this.boundingBox(),
            polygons = pyramid().pad(model.pad).value(function (d) {
            return d[field];
        }),
    ...
    doDraw: function doDraw(frame) {
        ...
        if (model.label && layout === 'heatmap') {
            ...
            labels.enter().append('text').classed('labels', true).attr("transform", function (d) {
                return 'translate(' + d.x + ', ' + d.y + ')';
            }).style("text-anchor", "middle").style("alignment-baseline", "middle").style("fill", fillLabel).style('font-size', fontSize).text(heatLabel).merge(labels).attr("transform", function (d) {
                return 'translate(' + d.x + ', ' + d.y + ')';
            }).style("fill", fillLabel).style('font-size', fontSize).text(heatLabel).call(textWrap, Math.ceil(0.8 * heat.size));
        }

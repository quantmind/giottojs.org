// d3-visualize Version 0.1.3. Copyright 2017 quantmind.com.
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('object-assign'), require('d3-array'), require('d3-random'), require('d3-collection'), require('d3-let'), require('d3-view'), require('d3-dsv'), require('d3-dispatch'), require('crossfilter'), require('d3-time-format'), require('d3-format'), require('d3-selection'), require('d3-transition'), require('d3-scale'), require('d3-axis'), require('d3-svg-legend'), require('d3-shape')) :
	typeof define === 'function' && define.amd ? define(['exports', 'object-assign', 'd3-array', 'd3-random', 'd3-collection', 'd3-let', 'd3-view', 'd3-dsv', 'd3-dispatch', 'crossfilter', 'd3-time-format', 'd3-format', 'd3-selection', 'd3-transition', 'd3-scale', 'd3-axis', 'd3-svg-legend', 'd3-shape'], factory) :
	(factory((global.d3 = global.d3 || {}),global.assign,global.d3,global.d3,global.d3,global.d3,global.d3,global.d3,global.d3,global.crossfilter,global.d3,global.d3,global.d3,global.d3,global.d3,global.d3,global.d3,global.d3));
}(this, (function (exports,assign,d3Array,d3Random,d3Collection,d3Let,d3View,d3Dsv,d3Dispatch,crossfilter,d3TimeFormat,d3Format,d3Selection,d3Transition,d3_scale,d3Axis,d3SvgLegend,d3_shape) { 'use strict';
...
var Svg = createPaper('svg', {
    initialise: function initialise(viz) {
        var svg = viz.visualParent.paper.append('svg').attr('id', viz.model.uid).classed(viz.visualType, true);
        return svg.node();
    }
});
...
vizPrototype.linearGradient = function (box, orientation) {
    var gid = d3View.viewUid();
    var defs = this.paper.select('defs');
    if (!defs.node()) defs = this.paper.append(defs);
    var grad = defs.append('linearGradient').attr('id', gid).attr('x1', '0%').attr('y1', '0%');
    ...
    doDraw: function doDraw(frame) {
        ...
        if (stacked) {
            ...
            var rects = bars.data(data).enter().append('g').classed('group', true).attr('fill', function (d) {
                ...
                return sz(d.key);
            }).attr('stroke', color.stroke).attr('stroke-opacity', color.strokeOpacity).attr('fill-opacity', color.fillOpacity).selectAll('rect').data(function (d) {
                return d;
            });
...
var area$1 = createChart('areachart', lineDrawing, {
    ...
    doDraw: function doDraw(frame) {
        var self = this,
            ...
            box = this.boundingBox(),
            paper = this.paper().size(box),
            areas = paper.group().attr("transform", this.translate(box.total.left, box.total.top)).selectAll('.area').data(info.data),
            lines = paper.group('lines').attr("transform", this.translate(box.total.left, box.total.top)).selectAll('.line').data(info.data),
            fill = this.fill(info.meta),
            curve = this.curve(model.curve);

        // TODO: fix this hack
        info.range.y[0] = 0;
        var line_ = d3_shape.line().x(this.x(box, info.range.x)).y(this.y(box, info.range.y)).curve(curve),
            area_ = d3_shape.area().x(this.x(box, info.range.x)).y1(this.y(box, info.range.y)).y0(this.y(box, info.range.y, constant(0))).curve(curve);

        lines.enter().append('path').attr('class', 'line').attr('fill', 'none').attr('stroke', color.stroke).attr('stroke-opacity', 0).attr('stroke-width', model.lineWidth).attr('d', line_).merge(lines).transition().attr('d', line_).attr('stroke', color.stroke).attr('stroke-opacity', color.strokeOpacity).attr('stroke-width', model.lineWidth);

        lines.exit().remove();

        areas.enter().append('path').attr('class', 'area').attr('fill', fill).attr('stroke', 'none').attr('stroke-width', 0).attr('d', area_).merge(areas).transition().attr('d', area_).attr('fill', fill).attr('fill-opacity', color.fillOpacity);

        areas.exit().remove();

        if (model.axisX) {
            var sx = this.getScale(model.scaleX).domain(info.range.x).range([0, box.innerWidth]),
                xa = this.axis(model.axisX, sx).ticks(this.ticks(box.innerWidth, 50)).tickFormat(this.format(info.range.x[0])).tickSizeOuter(model.axisTickSizeOuter);
            paper.group('x-axis').attr("transform", this.translate(box.total.left, box.total.top + box.innerHeight)).call(xa);
        }
        if (model.axisY) {
            var sy = this.getScale(model.scaleY).domain(info.range.y).range([box.innerHeight, 0]),
                ya = this.axis(model.axisY, sy).ticks(this.ticks(box.innerHeight, 30)).tickFormat(this.format(info.range.y[0])).tickSizeOuter(model.axisTickSizeOuter);
...
var pie$1 = createChart('piechart', proportional, {
    ...
    doDraw: function doDraw(frame) {
        ...
        slices.enter().append('path').attr('class', 'slice').attr('stroke', color.stroke).attr('stroke-opacity', 0).attr('fill-opacity', 0).attr('fill', fill).attr('stroke-width', model.lineWidth).merge(slices).transition().attr('stroke', color.stroke).attr('stroke-opacity', color.strokeOpacity).attr('d', arcs).attr('fill', fill).attr('fill-opacity', color.fillOpacity);

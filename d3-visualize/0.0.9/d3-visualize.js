// d3-visualize Version 0.0.9. Copyright 2017 quantmind.com.
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('d3-canvas-transition'), require('object-assign'), require('d3-array'), require('d3-random'), require('d3-collection'), require('d3-let'), require('d3-view'), require('d3-dsv'), require('d3-dispatch'), require('crossfilter'), require('d3-format'), require('d3-time-format'), require('d3-selection'), require('d3-scale'), require('d3-shape')) :
	typeof define === 'function' && define.amd ? define(['exports', 'd3-canvas-transition', 'object-assign', 'd3-array', 'd3-random', 'd3-collection', 'd3-let', 'd3-view', 'd3-dsv', 'd3-dispatch', 'crossfilter', 'd3-format', 'd3-time-format', 'd3-selection', 'd3-scale', 'd3-shape'], factory) :
...
}(this, (function (exports,d3CanvasTransition,assign,d3Array,d3Random,d3Collection,d3Let,d3View,d3Dsv,d3Dispatch,crossfilter,d3Format,d3TimeFormat,d3Selection,d3Scale,d3_shape) { 'use strict';
...
colorScales.set('viridis', function () {
    return d3Scale.scaleSequential(d3Scale.interpolateViridis);
});
colorScales.set('inferno', function () {
    return d3Scale.scaleSequential(d3Scale.interpolateInferno);
});
colorScales.set('magma', function () {
    return d3Scale.scaleSequential(d3Scale.interpolateMagma);
});
colorScales.set('plasma', function () {
    return d3Scale.scaleSequential(d3Scale.interpolatePlasma);
});
colorScales.set('warm', function () {
    return d3Scale.scaleSequential(d3Scale.interpolateWarm);
});
colorScales.set('cool', function () {
    return d3Scale.scaleSequential(d3Scale.interpolateCool);
});
...
colorScales.set('cubehelix', function () {
    return d3Scale.scaleSequential(d3Scale.interpolateCubehelixDefault);
});
...
var proportional = {
    ...
    proportionalData: function proportionalData(frame, field) {
        return frame.dimension(function (d) {
            return d[field];
        }).top(Infinity);
    }
};
...
var pie$1 = createChart('piechart', proportional, {
    ...
    doDraw: function doDraw(frame) {
        ...
        data = angles(this.proportionalData(frame, field)),
            fill = this.fill(data),
            slices = paper.sel.attr('width', box.width).attr('height', box.height).append('g').attr("transform", this.translate(box.total.left + box.innerWidth / 2, box.total.top + box.innerHeight / 2)).selectAll('.slice').data(data);

        slices.enter().append('path').attr('class', 'slice').attr('stroke', model.color).attr('stroke-opacity', 0).attr('fill-opacity', 0).attr('fill', fill).attr('stroke-width', model.lineWidth).merge(slices)
        //.transition(update)
        .attr('stroke', model.color).attr('stroke-opacity', model.colorOpacity).attr('d', arcs).attr('fill', fill).attr('fill-opacity', model.fillOpacity);
...
var pyramid$1 = createChart('pyramidchart', proportional, {
    ...
    doDraw: function doDraw(frame) {
        var model = this.getModel(),
            ...
            fill = this.fill(data),
            paper = this.paper(),
            segments = paper.sel.attr("transforms", this.translate(box.shift.left + box.innerWidth / 2, box.shift.top)).selectAll('.segment').data(polygons(data));

        segments.enter().append('path').attr('class', 'segment').attr('stroke', color.stroke).attr('stroke-opacity', 0).attr('fill-opacity', 0).attr('fill', fill).attr('stroke-width', paper.dim(model.lineWidth)).merge(segments)
        //.transition(update)
        .attr('stroke', color.stroke).attr('stroke-opacity', model.strokeOpacity).attr('d', marks).attr('fill', fill).attr('fill-opacity', color.fillOpacity);

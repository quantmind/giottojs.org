// d3-visualize Version 0.0.15. Copyright 2017 quantmind.com.
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('object-assign'), require('d3-array'), require('d3-random'), require('d3-collection'), require('d3-let'), require('d3-view'), require('d3-dsv'), require('d3-dispatch'), require('crossfilter'), require('d3-time-format'), require('d3-format'), require('d3-selection'), require('d3-transition'), require('d3-scale'), require('d3-svg-legend'), require('d3-shape')) :
	typeof define === 'function' && define.amd ? define(['exports', 'object-assign', 'd3-array', 'd3-random', 'd3-collection', 'd3-let', 'd3-view', 'd3-dsv', 'd3-dispatch', 'crossfilter', 'd3-time-format', 'd3-format', 'd3-selection', 'd3-transition', 'd3-scale', 'd3-svg-legend', 'd3-shape'], factory) :
	(factory((global.d3 = global.d3 || {}),global.assign,global.d3,global.d3,global.d3,global.d3,global.d3,global.d3,global.d3,global.crossfilter,global.d3,global.d3,global.d3,global.d3,global.d3,global.d3,global.d3));
}(this, (function (exports,assign,d3Array,d3Random,d3Collection,d3Let,d3View,d3Dsv,d3Dispatch,crossfilter,d3TimeFormat,d3Format,d3Selection,d3Transition,d3_scale,d3SvgLegend,d3_shape) { 'use strict';
...
var camelFunction = function (o, prefix, name) {
...
function menuTitle(viz, title) {
    ...
    viz.menu.select('.title').html(title.text).style('font-size', size + 'px').style('line-height', height + 'px');
}
...
vizPrototype.colors = function (n) {
    ...
    return d3Array.range(1, n + 1).map(function (v) {
        return scale(v);
    });
};
...
vizPrototype.legend = function (cfg, box) {
    ...
    for (var key in cfg) {
        legend[key](cfg[key]);
    }var g = this.paper().group('legend').call(legend),
...
var VisualContainer = createVisual('container', {
    ...
    draw: function draw() {
        visuals.events.call('before-draw', undefined, this);
        this.live.forEach(function (visual) {
            visual.draw();
        });
        visuals.events.call('after-draw', undefined, this);
    },
...
var line$1 = createChart('linechart', lineDrawing, {
    ...
    x: function x(box, ranges) {
        var model = this.getModel(),
            scale = self.getScale(model.scaleX).domain(d3Array.extent(ranges)).range([0, box.innerWidth]);
        return function (d) {
    ...
    y: function y(box, ranges) {
        var model = this.getModel(),
            scale = self.getScale(model.scaleY).domain(d3Array.extent(ranges)).range([box.innerHeight, 0]);
        return function (d) {
...
var bar = createChart('barchart', lineDrawing, {
    ...
    doDraw: function doDraw(frame) {
        ...
        data = frame.data,
            box = this.boundingBox(),
            paper = this.paper(),
            bars = paper.size(box).group().attr("transform", this.translate(box.total.left, box.total.top)).selectAll('.group'),
            x = model.x,
            y = model.y,
        ...
        if (stacked) {
            ...
            var rects = bars.data(data).enter().append('g').classed('group', true).attr('fill', function (d) {
                return sz(d.key);
            }).merge(bars).selectAll('rect').data(function (d) {
                return d;
            });
            rects.enter().append('rect').merge(rects).attr('x', xrect).attr('y', yrect).attr('height', height).attr('width', width);
        } else {
...
var pyramid$1 = createChart('pyramidchart', proportional, {
    ...
    doDraw: function doDraw(frame) {
        ...
        this.legend({
            scale: d3_scale.scaleOrdinal().domain(['a', 'b', 'c', 'd', 'e', 'f']).range(fill.colors)
        }, box);
    }
});

// d3-view Version 1.3.1. Copyright 2018 quantmind.com.
...
var viewEvents = d3Dispatch.dispatch('message', 'component-created', 'component-mount', 'component-mounted', 'component-error', 'directive-refresh');
...
var attributes = function attributes(element) {
    var attrs = {};
    var attr = void 0;
...
var reactiveParentProperty = function reactiveParentProperty(key, value) {
    return {
        reactOn: [value],
        get: function get() {
            return this[value];
...
Directives.prototype = {
    ...
    once: function once(model, data) {
        var dir = void 0;
        var all = [];
        for (var i = 0; i < this.all.length; ++i) {
            dir = this.all[i];
            if (dir.once) dir.once(model, data);else all.push(dir);
        }
        this.all = all;
    }
};
...
function mountElement(element, vm, data, onMounted) {
    ...
    if (preMount) return preMount.execute(vm.model);else {
        var promise = void 0;
        if (component) promise = component({ parent: vm }).mount(element, data, onMounted);else promise = Promise.all(slice(element.children).map(function (c) {
...
var d3For = {
    ...
    refresh: function refresh(model, items) {
        ...
        entries.enter().append(function () {
            ...
            x = { index: index };
            x[itemName] = d;
            mounted$$1.push(forComponent({
                model: x,
                parent: vm
            }).mount(this).then(function (fv) {
        ...
        Promise.all(mounted$$1).then(function () {
...
var coreComponents = extendComponents(new Map(), components);

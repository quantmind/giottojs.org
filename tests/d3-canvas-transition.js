...
var methods = ['radius', 'innerRadius', 'outerRadius', 'cornerRadius', 'x0', 'x1', 'x', 'y0', 'y1', 'y'];

var path = function (p, factor) {
    ...
    for (var i = 0; i < methods.length; ++i) {
        wrap(cp, p, factor, methods[i]);
    }wrap(cp, p, factor * factor, 'size');
...
function wrap(cp, p, factor, method) {
    if (!p[method]) return;
    var original = p[method]();

    p[method](wrapped);

    cp[method] = function (_) {
    ...
    function wrapped() {
        return factor * original.apply(this, arguments);
    }
}

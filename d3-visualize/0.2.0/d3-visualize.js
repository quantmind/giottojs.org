...
var Svg = createPaper('svg', {
    initialise: function initialise(viz) {
        ...
        if (!svg.size()) svg = viz.visualParent.paper.append('svg').attr('id', viz.visualParent.model.uid).classed(viz.visualType, true).style('position', 'absolute');
        return svg.node();
    }
});
...
vizPrototype.xAxis = function (scale, x, y) {
    ...
    this.paper().group('x-axis').attr("transform", this.translate(x, y)).transition().call(axis).select('path.domain').attr('stroke', model.stroke);
};

vizPrototype.yAxis = function (scale, x, y) {
    var model = this.getModel('yAxis'),
        axis = getAxis(model.location, scale, model);
    this.paper().group('y-axis').attr("transform", this.translate(x, y)).transition().call(axis).select('path.domain').attr('stroke', model.stroke);
};

vizPrototype.xAxis1 = function (location, scale, box, value) {
    var model = this.getModel('xAxis'),
        axis = getAxis(location, scale, model, value);
    this.paper().group('x-axis').attr("transform", this.translateAxis(location, box)).transition().call(axis).select('path.domain').attr('stroke', model.stroke);
    if (model.title) this.axisTitle(location, scale, box, model);
};

vizPrototype.yAxis1 = function (location, scale, box, value) {
    var model = this.getModel('yAxis'),
        axis = getAxis(location, scale, model, value);
    this.paper().group('y-axis').attr("transform", this.translateAxis(location, box)).transition().call(axis).select('path.domain').attr('stroke', model.stroke);
    if (model.title) this.axisTitle(location, scale, box, model);
};
...
vizPrototype.axisTitle = function (location, scale, box, model) {
    var rotate = model.titleRotate || 0,
        x = 0,
        y = 0,
        title = this.group('axis-' + location + '-title').attr("transform", this.translateAxis(location, box)).selectAll('text').data([model.title]);
    ...
    title.enter().append('text').attr("transform", translate).style("text-anchor", "middle").style("font-size", font).style("fill", model.stroke).text(function (d) {
        return d;
...
vizPrototype.translateAxis = function (location, box) {
    if (location === 'top' || location === 'left') return this.translate(box.total.left, box.total.top);else if (location === 'bottom') return this.translate(box.total.left, box.total.top + box.innerHeight);else return this.translate(box.total.left + box.innerWidth, box.total.top);
};
    ...
    doDraw: function doDraw(frame) {
        ...
        if (stacked) {
            ...
            rects = bars.data(data).enter().append('g').classed('group', true).attr('fill', function (d) {
                ...
                return sz(d.key);
            }).attr('stroke', color$$1.stroke).attr('stroke-opacity', color$$1.strokeOpacity).attr('fill-opacity', color$$1.fillOpacity).selectAll('rect').data(stackedData);
            ...
            //
            //  Grouped bar chart
            var x1 = this.getScale('band').domain(groups).paddingInner(0.5 * model.padding);

            // set the value domain
            sy.domain([0, d3Array.max(frame.data, function (d) {
                return d[y];
            })]).nice();
            ...
            rects = bars.enter().append('g').classed('group', true).attr("transform", function (d) {
                return _this.translate(xrect(d), 0);
            ...
            rects.enter().append('rect').attr('x', function (d) {
                return x1(d.key);
            }).attr('y', gy).attr('height', height).attr('width', width).attr('stroke', color$$1.stroke).attr('stroke-opacity', 0).attr('fill-opacity', 0).attr('fill', function (d) {
                return sz(d.key);
            }).on("mouseover", this.mouseOver()).on("mouseout", this.mouseOut()).merge(rects).transition().attr('x', function (d) {
                return x1(d.key);
            }).attr('y', gy).attr('height', height).attr('width', width).attr('stroke', color$$1.stroke).attr('stroke-opacity', color$$1.strokeOpacity).attr('fill-opacity', color$$1.fillOpacity).attr('fill', function (d) {
                return sz(d.key);
            });
        ...
        if (model.orientation === 'vertical') {
            ...
            if (model.axisY) this.yAxis1(model.axisY === true ? "bottom" : model.axisY, sy, box);
        }
...
var area$1 = createChart('areachart', lineDrawing, {
    ...
    doDraw: function doDraw(frame) {
        var self = this,
            ...
            areas = this.group().attr("transform", this.translate(box.total.left, box.total.top)).selectAll('.areagroup').data(info.data),
...
var pyramid$1 = createChart('pyramidchart', proportional, {
    ...
    doDraw: function doDraw(frame) {
        var model = this.getModel(),
            ...
            fill = this.fill(data),
            paper = this.paper(),
            segments = paper.size(box).group().attr("transform", this.translate(box.total.left + box.innerWidth / 2, box.total.top)).style("shape-rendering", "crispEdges").selectAll('.segment').data(data);
...
var treemap = createChart('treemap', {
    ...
    doDraw: function doDraw(frame, d3) {
        ...
        }).on("mouseover", this.mouseOver()).on("mouseout", this.mouseOut()).merge(rects).transition().attr("width", function (d) {
            return d.x1 - d.x0;
        }).attr("height", function (d) {
            return d.y1 - d.y0;
        }).attr("fill", function (d) {
        ...
        rects.enter().append("text").style('fill', function (d) {
            return colorContrast(colors[d.data._counter]);
...
var text = createChart('text', {
    ...
    doDraw: function doDraw(frame) {
        var self = this,
            ...
            box = this.boundingBox(),
            size = this.font(box),
            group = this.group().attr("transform", this.translate(box.total.left, box.total.top)).selectAll('text').data(frame.data),
...
var heatmap = createChart('heatmap', lineDrawing, {
    ...
    doDraw: function doDraw(frame) {
        ...
        shapes.enter().append('path').classed('shape', true).attr("transform", function (d) {
            ...
            return d.color;
        }).attr("fill-opacity", 0).attr("stroke-opacity", 0).attr("stroke", color$$1.stroke).attr('d', shape).on("mouseover", this.mouseOver()).on("mouseout", this.mouseOut()).merge(shapes).transition().attr("transform", function (d) {
            return 'translate(' + d.x + ', ' + d.y + ')';

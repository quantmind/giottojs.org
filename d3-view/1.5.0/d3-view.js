// d3-view v1.5.0 Copyright 2018 quantmind.com
(function (global, factory) {
typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('d3-let'), require('d3-selection'), require('d3-transition'), require('d3-timer'), require('d3-dispatch')) :
typeof define === 'function' && define.amd ? define(['exports', 'd3-let', 'd3-selection', 'd3-transition', 'd3-timer', 'd3-dispatch'], factory) :
(factory((global.d3 = global.d3 || {}),global.d3,global.d3,global.d3,global.d3,global.d3));
}(this, (function (exports,d3Let,d3Selection,d3Transition,d3Timer,d3Dispatch) { 'use strict';
...
var providers = {
  ...
  fetch: d3Let.inBrowser ? window.fetch : null
};
...
var warn = msg => providers.logger.warn(`${prefix} ${msg}`);
...
var base = {
  ...
  select(selector) {
    if (typeof selector === "string")
      return d3Selection.select(this.ownerDocument || document).select(selector);
    return d3Selection.select(selector);
  },
  //
  selectAll(selector) {
    if (typeof selector === "string")
      return d3Selection.select(this.ownerDocument || document).selectAll(selector);
...
var formGroup = (field, wrappedEl, fieldEl) => {
  var data = field.props,
...
const groupTpl$2 = `<div class="form-group">
<slot></slot>
</div>`;

function submit(field, wrappedEl, fieldEl) {
  var data = field.props,
...
const formElement = {
  ...
  init(el) {
    ...
    properties.forEach((prop, key) => {
      ...
      if (value) {
        if (d3Let.isString(value)) el.attr(`d3-attr-${key}`, value);
...
const asOptions = options => {
  ...
  options.forEach((opt, idx) => {
    if (d3Let.isArray(opt))
      opt = {
        value: opt[0],
        label: opt[1] || opt[0]
      };
    else if (d3Let.isString(opt))
      opt = {
        value: opt,
        label: opt
      };
...
//
// Submit element
var submit$2 = d3Let.assign({}, formElement, {
  model: {
    $submit() {
...
var ddispatch = () => {
  ...
  const events = d3Dispatch.dispatch("change");
...
var slice = (obj, idx = 0) => Array.prototype.slice.call(obj, idx);
...
const isModel = value =>
  d3Let.isObject(value) && value.toString() === "[object d3Model]";
...
const reactive = (model, key, value) => {
  ...
  function typeValue(newValue, oldValue) {
    ...
    else if (d3Let.isArray(newValue)) return arrayValue(newValue, oldValue);
...
function createDirective(obj) {
  ...
  Directive.prototype = { ...base$1, ...prototype, ...obj };
...
const protoComponent = {
  ...
  mount(el, data) {
    ...
    this.ownerDocument = el.ownerDocument;
    //
    const directives = sel$$1.directives(this);

    let props = d3Let.assign(dataAttributes(directives.attrs), sel$$1.datum(), data),
      extra = maybeJson(d3Let.pop(props, "props")),
      value,
      model$$1,
      parentModel;

    if (d3Let.isObject(extra)) props = d3Let.assign(extra, props);
    else if (this.parent && this.parent.props[extra])
      props = d3Let.assign({}, this.parent.props[extra], props);
    ...
    if (this.parent) {
      ...
      model$$1 = d3Let.pop(props, "model");
      if (model$$1 && !model$$1.$child) model$$1 = parentModel[model$$1];
      if (!model$$1) model$$1 = parentModel.$new();
      else model$$1 = model$$1.$child();
      //
    ...
    Object.keys(this.model).forEach(key => {
      ...
      if (value !== undefined) {
        if (d3Let.isString(value) && parentModel && parentModel.$isReactive(value))
          model$$1.$connect(key, value, parentModel);
        else model$$1.$set(key, maybeJson(value));
      } else if (model$$1[key] === undefined) model$$1.$set(key, this.model[key]);
    });
    this.model = bindView(this, model$$1);
    //
    // create the new element from the render function
    if (!props.id) props.id = model$$1.uid;
    this.props = props;
    //
  ...
  createElement(tag, props) {
    ...
    const sel$$1 = this.select(doc.createElement(tag));
    if (props) {
  ...
  doMount(el) {
    ...
    return newEl
      .then(element => compile$1(this, el, element))
      .catch(exc => error$1(this, el, exc));
  },
...
const createComponent = (name, o, coreDirectives, coreComponents) => {
  ...
  function Component(options) {
    ...
    this.model = asObject(model$$1, d3Let.pop(options, "model"));
    this.events.call("created", undefined, this);
  }

  Component.prototype = d3Let.assign({}, base$1, protoComponent, obj);
...
const extendComponents = (container, components) => {
...
const extendDirectives = (container, directives) => {
...
const compile$1 = (cm, origEl, element) => {
...
const bindView = (view, model$$1) => {
  Object.defineProperties(model$$1, {
    $$view: {
      get() {
        return view;
      }
...
function createValueType(proto) {
  ...
  ValueType.prototype = d3Let.assign({}, base$2, proto);
...
exports.viewBootstrapForms = plugin;
exports.viewForms = plugin$1;
exports.view = main$1;
